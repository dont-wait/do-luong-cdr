...
export class CloService {
  ...
  public async createClo(createCloDto: CreateCloDto | CreateCloDto[]) {
    try {
      if (!Array.isArray(createCloDto)) {
        const { clo_name, clo_content, clo_parent_id, subject_id, ploIds } =
          createCloDto;
        ...
        const results: {
          ...
          clo_name: string;
          clo_content: string;
          clo_parent_id: string | undefined;
          subject_id: string;
          ploIds: string[];
        }[] = [];
          ...
          if (ploIds && ploIds.length > 0) {
            ...
            if (validPlos.length !== ploIds.length) {
              throw new BadRequestException(
                `Some Plo IDs are invalid for ${clo_name}`,
              );
            }
          }
  ...
  async getCloById(id: string) {
    ...
    if (!cloId) throw new BadRequestException(`Clo with ${id} NOT FOUND`);
    return cloId;
  }
